...
const Body=()=>{
    ...
    const fetchData= async()=>{
        const data=await fetch("https://www.swiggy.com/dapi/restaurants/list/v5?lat=24.4910988&lng=86.6888051&is-seo-homepage-enabled=true&page_type=DESKTOP_WEB_LISTING");
    ...
    if(!onlineStatus) return <h1>Looks like you're Offline!! Check your Interner!</h1>
    return resList?.length===0?<Shimmer />:(
        <div className="body">
            <div className="flex justify-between">
                <div className="m-4 p-4 ">
                    <button className=" rounded-lg px-4 py-2 bg-green-300" onClick={TopList}>Top Rated Restaurant</button>  
                </div>
                <div className="m-4 p-4 w-2/4 flex justify-end">
                    <input type="text" className="box-border w-3/4 rounded-l-lg px-4 py-2 border border-solid border-black " value={searchText} onChange={(e)=>setSearchText(e.target.value)}/>
                    <button className=" rounded-r-lg px-4 py-2 mx-2 bg-green-300 " onClick={()=>{
            ...
            <div className="flex flex-wrap mx-4">
                {filteredList?.map((resData)=>(
                    <Link key={resData.info.id} to={`/restaurant/${resData.info.id}`}>
                        {resData.info.promoted? <ResPromotedCard resData={resData} res={"gi"} />:<ResCard  resData={resData}/>}
                        </Link>
                ))}
            </div>
        </div>
